...
from dbbackup.tests.utils import FakeStorage, DEV_NULL
...
class MediabackupBackupMediafilesTest(TestCase):
    ...
    def test_func(self):
        ...
        self.assertEqual(1, len(self.command.storage.written_files))

    def test_compress(self):
        self.command.backup_mediafiles(encrypt=False, compress=True)
        self.assertEqual(1, len(self.command.storage.written_files))
        self.assertTrue(self.command.storage.written_files[0][0].endswith('.gz'))
    ...
    def test_encrypt(self):
        ...
        self.assertEqual(1, len(self.command.storage.written_files))
        outputfile = self.command.storage.written_files[0][1]
        outputfile.seek(0)
